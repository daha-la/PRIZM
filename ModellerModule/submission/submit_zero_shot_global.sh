...
# - DONE markers keyed by MSA_filename (reuse training across datasets/libraries)
...
# ============================ Model registry =================================
# model -> script, env, needs_eve, needs_eunirep
#
# Envs rules:
# - VenusREM -> venusrem
# - ProSST   -> prosst
# - MULAN    -> mulan
# - UniRep*  -> unirep_evotune (train/score)
# - ProtSSN  -> protssn
# - RSALOR   -> rsalor
# - else     -> PRIZM
#
declare -A MODEL_TO_SCRIPT=(
  ...
  ["mift"]="scoring_MIFST_substitutions.sh"
...
# - EVE provides MSA weights + trained EVE model used by these
EVE_DEPENDENTS=(
  "gemme"
  "msa_transformer"
  "rsalor"
...
# - eUniRep required for eUniRep scoring
EUNIREP_DEPENDENTS=(
  "unirep_evotune_scoring"
)
...
# ============================ GPU cost model =================================
# (kept as its own block, per your preference)
declare -A GPU_COST=(
  # Low cost models
  ["gemme"]=0
  ["rsalor"]=1
...
# (smallest → largest; strict order; no reordering at runtime)
...
generate_tmp_tsv() {
  TMP_TSV="$(mktemp -t prizm_msa_map_XXXXXX.tsv)"
  python "$HELPER_PY" \
...
# DONE marker dirs keyed by MSA_filename (human-readable)
...
eve_done_file() {
  local msa="$1"
  echo "${EVE_DONE_BASE}/${msa}/done.txt"
}

eunirep_done_file() {
  local msa="$1"
  echo "${EUNIREP_DONE_BASE}/${msa}/done.txt"
}
...
# ============================ Conda runner ===================================
# Uses conda run (recommended for non-interactive / nohup).
run_in_env() {
  local env="$1"; shift
  conda run -n "$env" "$@"
}
...
# ============================ Execution: training =============================
# Training is always sequential (not background). DONE markers keyed by MSA_filename.
run_training_phase() {
  ...
  # Build the MSA map TSV once and reuse it for both training phases
  ...
  while IFS=$'\t' read -r msa rep_idx; do
    ...
    local donefile
    if [[ "$phase" == "EVE" ]]; then
      donefile="$(eve_done_file "$msa")"
    else
      donefile="$(eunirep_done_file "$msa")"
    fi

    if ! $OVERWRITE_TRAINING && [[ -f "$donefile" ]]; then
      echo "✓ ${phase} skip (DONE exists): msa=${msa}"
      continue
    fi

    echo "→ ${phase} train: msa=${msa} (rep_index=${rep_idx}, env=${env})"
    # Call existing training script for ONLY the representative index
    # (rep_idx..rep_idx), so it trains once for this MSA.
...
run_scoring_model() {
  ...
  if [[ "$MODE" == "background" ]]; then
    ...
    # Launch with nohup; ensure conda env is used via conda run
    (
      cd "$SCORING_DIR"
      nohup bash -lc "conda run -n \"$env\" bash \"$script\" \"$FIRST_INDEX\" \"$LAST_INDEX\" > \"$out\" 2> \"$err\"" \
        > /dev/null 2>&1 &
      echo $!
    ) > "${out}.pid"
    ...
    ( cd "$SCORING_DIR" && run_in_env "$env" bash "$script" "$FIRST_INDEX" "$LAST_INDEX" > "$out" 2> "$err" )
  fi
}
...
if $DRY_RUN; then
  ...
  echo
  echo "Training needed?"
  echo "  - EVE:    $NEED_EVE"
  echo "  - eUniRep:$NEED_EUNIREP"
  echo
  echo "DONE marker bases:"
  echo "  - EVE:    $EVE_DONE_BASE/<MSA_filename>/done.txt"
  echo "  - eUniRep:$EUNIREP_DONE_BASE/<MSA_filename>/done.txt"
  echo
  echo "Helper:"
...
for m in "${SCORING_ORDER[@]}"; do
  # Safety: don't allow training keys in scoring selection
