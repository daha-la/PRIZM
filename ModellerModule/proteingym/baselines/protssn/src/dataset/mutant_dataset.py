...
class MutantDataset(Dataset):
    ...
    def generate_protein_graph_evaluation(self):
        ...
        self.total_protein_names = sorted(
            [
                name for name in os.listdir(self.raw_dir)
        ...
        for name in process_bar:
            ...
            # if graph already exists, don't rebuild it
        ...
        # graphs that actually exist on disk
        all_graph_stems = [
            name.split(".")[0] for name in os.listdir(self.saved_graph_path)
            if name.endswith(".pt")
        ]

        # but only keep graphs whose protein still exists in raw_dir
        valid_stems = sorted(
            [stem for stem in all_graph_stems if stem in self.total_protein_names]
        )

        self.protein_names = valid_stems
        with open(self.saved_protein_name_file, 'w') as fp:
            for item in self.protein_names:
                fp.writelines("%s\n" % item)
