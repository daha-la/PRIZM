...
class EGNN_Sparse(MessagePassing):
    ...
    def propagate(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        msg_kwargs = {k: coll_dict[k] for k in self.message.__code__.co_varnames if k in coll_dict} #self.inspector.distribute('message', coll_dict)
        aggr_kwargs = {k: coll_dict[k] for k in self.aggregate.__code__.co_varnames if k in coll_dict}#self.inspector.distribute('aggregate', coll_dict)
        update_kwargs = {k: coll_dict[k] for k in self.update.__code__.co_varnames if k in coll_dict}#self.inspector.distribute('update', coll_dict)
