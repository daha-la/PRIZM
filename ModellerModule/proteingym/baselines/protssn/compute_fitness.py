...
def label_row(rows, sequence, token_probs, offset_idx=1):
    ...
    for row in rows.split(sep):
        ...
        try:
            ...
            raise ValueError
        print(f"row: {row}, sequence: {sequence}, wt: {wt}, idx: {idx}, mt: {mt}, wt from seq: {sequence[idx]}")
...
def predict(args, plm_model, gnn_model, loader):
    ...
    target_id = dataset_reference.loc[args.DMS_index]['DMS_id']
    found = False

    with torch.no_grad():
        for data in loader:
            #if args.DMS_index is not None:
            #    if dataset_reference.loc[args.DMS_index]['DMS_id'] != data.protein_name[0]:
            #        continue
    ...
    if not found:
        raise RuntimeError(
            f"\n\nERROR: DMS_id '{target_id}' not found in mutant_dataset_dir:\n"
            f"    {args.mutant_dataset_dir}\n"
            "Make sure the dataset folder exists and the folder name EXACTLY "
            "matches the DMS_id in dataset_reference.\n"
        )
...
def ensemble(args):
    ...
    dms_id = dataset_reference.loc[args.DMS_index]['DMS_id']
    result_file = os.path.join(args.output_scores_folder, f"{dms_id}.csv")

    if not os.path.exists(result_file):
        raise RuntimeError(
            f"Ensemble failed: no result file found for {dms_id}.\n"
            f"Expected at: {result_file}\n"
        )

    df = pd.read_csv(result_file)

    model_cols = [c for c in df.columns if c.startswith("ProtSSN")]
    if not model_cols:
        raise RuntimeError(
            f"Ensemble failed: no ProtSSN model columns found in {result_file}\n"
            f"Columns: {df.columns.tolist()}\n"
        )

    df["ProtSSN_ensemble"] = df[model_cols].mean(axis=1)
    df.to_csv(result_file, index=False)


    # Ugly hack allow for the addition of new proteins later. Should be handled more gracefully in later versions.
    # protssn_root = args.mutant_dataset_dir
    # state_files = ["saved_proteins.txt", "total_proteins.txt", "wrong_proteins.txt"]
    # for k in [10, 20, 30]:
    #     model_dir = os.path.join(protssn_root, f"Protssn_k{k}")
    ...
    for k in [10, 20, 30]:
        ...
        # Walk bottom-up so we delete files before their parent dirs
