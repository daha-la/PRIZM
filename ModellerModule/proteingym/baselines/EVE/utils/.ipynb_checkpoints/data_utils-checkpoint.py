...
def get_one_hot_3D_fn(msa_data):
        ...
        def fn(batch):
            one_hot_out = np.zeros((len(batch), msa_data.seq_len, len(msa_data.alphabet)))
...
class InfiniteDataLoader(DataLoader):
    ...
    def __next__(self):
        try:
            ...
            self.iter_loader = super().__iter__()
            #self.iter_loader = iter(self)  # Reinitialize with self
            batch = next(self.iter_loader)
        return batch
    
    #def one_hot_fn(self):
    #    # Define one_hot_fn here using self.msa_data and self.some_parameters
    #    return get_one_hot_3D_fn(self.msa_data)

    #def collate_fn(self, batch):
    #    # Construct a batch of one-hot-encodings
    #    one_hot_fn = get_one_hot_3D_fn(self.msa_data)
    #    batch_seq_tensor = one_hot_fn(batch)
    #    if isinstance(batch_seq_tensor, list):
    #        batch_seq_tensor = torch.stack(batch_seq_tensor, dim=0)
...
def get_dataloader(msa_data: MSA_processing, batch_size, num_training_steps):
    ...
    def collate_fn(batch):
       # Construct a batch of one-hot-encodings
        batch_seq_tensor = one_hot_fn(batch)
        return batch_seq_tensor
    #functions = InfiniteDataLoader(msa_data, dataset)
    ...
    dataloader = InfiniteDataLoader(
        ...
        sampler=sampler, 
        #collate_fn=functions.collate_fn,)
