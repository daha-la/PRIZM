...
def main():
    ...
    '''
    for i in range(len(dataset_reference.index)):
        dataset_entry = dataset_reference.loc[i]
        if args.DMS_index == i:
            # Init metadata
            dms_name = dataset_entry["DMS_id"]
            print(f"\nRun RSALOR on {dms_name} dataset ...")
            ...
            for property_to_remove in REMOVE_PROPERTIES_IN_OUTPUT:
                ...
                if property_to_remove in dataset.columns:
                    dataset.drop(columns=[property_to_remove], inplace=True)
            ...
            # Assign predicted values
            dataset[PREDICTION_PROPERTY] = 0.0
            if args.include_LOR:
                dataset["LOR"] = 0.0

            # Calculate the predicted values
            for index, row in dataset.iterrows():
                mutations_arr = row[MUTATION_PROPERTY].split(":")
                dataset.at[index, PREDICTION_PROPERTY] = sum(
                    [rsalor_single_scores_map[mut]["RSA*LOR"] for mut in mutations_arr]
                )
                if args.include_LOR:
                    dataset.at[index, "LOR"] = sum(
                        [rsalor_single_scores_map[mut]["LOR"] for mut in mutations_arr]
                    )
