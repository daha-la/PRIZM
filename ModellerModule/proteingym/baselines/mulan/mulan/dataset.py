...
class ProteinDataset(Dataset):
    def __init__(self, protein_data_path, 
        ...
        self.sequences, self.angles, self.protein_names = self.tokenizer.load_dataset(saved_dataset_path, 
                                                                                      split)
        print('protein_names', self.protein_names)
...
def data_collate_fn_dynamic(protein_parts_tuple, esm_tokenizer, nan_value, 
                         ...
                         max_prot_len=3000, #changed from 100000000000000,
                         predict_contacts='none',
    ...
    if use_foldseek_sequences:
        struct_inputs_pad = struct_inputs[:,0,:].unsqueeze(1).expand(-1, 1, -1) 
        struct_inputs_nopad = struct_inputs[:,1:-1,:]
        struct_inputs_nopad = struct_inputs_nopad.repeat_interleave(2, dim=1)
        struct_inputs = torch.cat([struct_inputs_pad, struct_inputs_nopad, struct_inputs_pad], dim=1)
        
        plddts = plddts.repeat_interleave(2, dim=1)
    ...
    # Downsample tokenized inputs by removing every second token
    #inputs = inputs[:, ::2]
    labels = None
    if mask_inputs:
    ...
    #expanded_plddts = plddts.repeat_interleave(2, dim=1)
    #padded_plddts[:, 1:-1] = expanded_plddts
