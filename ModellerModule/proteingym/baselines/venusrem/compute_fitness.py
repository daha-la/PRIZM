...
if __name__ == "__main__":
    ...
    parser.add_argument("--DMS_index", type=int, help="DMS Index of protein of interest",)
    ...
    for model_idx, model_name in enumerate(args.model_name):
        os.makedirs(f"{args.output_scores_folder}/{model_name.split('-')[-1]}", exist_ok=True)
        ...
        if os.path.exists(f"{args.struc_seq_dir}/{model_name.split('-')[-1]}/{dataset_entry['pdb_file'].split('.')[0]}.fasta"):
            ...
            print(f"python baselines/venusrem/venusrem/data/get_struc_seq.py --pdb_file {pdb_file} --vocab_size {model_name.split('-')[-1]} --out_dir {args.struc_seq_dir}")
            subprocess.run(f"python baselines/venusrem/venusrem/data/get_struc_seq.py --pdb_file {pdb_file} --vocab_size {model_name.split('-')[-1]} --out_dir {args.struc_seq_dir}", shell=True, executable="/bin/bash")
            structure_fasta = f"{args.struc_seq_dir}/{model_name.split('-')[-1]}/{dataset_entry['pdb_file'].split('.')[0]}.fasta"
        mutant_file = f"{args.mutant_dir}/{dataset_entry['DMS_filename']}"
        msa_file = f"{args.aa_seq_aln_dir}/{dataset_entry['MSA_filename']}"
        if args.logit_mode is not None:
            if "aa_seq_aln" in args.logit_mode:
                aa_seq_aln_file = msa_file
                print(f">>> Using the following MSA: {aa_seq_aln_file}")
                '''
        ...
        if os.path.exists(f"{args.output_scores_folder}/{model_name.split('-')[-1]}/{protein_name}.csv"):
            mutant_file = f"{args.output_scores_folder}/{model_name.split('-')[-1]}/{protein_name}.csv"
        ...
        print("="*50)'
        '''
        mutant_df.to_csv(f"{args.output_scores_folder}/{model_name.split('-')[-1]}/{protein_name}.csv", index=False)
