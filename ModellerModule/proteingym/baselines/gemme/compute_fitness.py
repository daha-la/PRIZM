...
if __name__ == "__main__":
    ...
    "conda activate GEMME && "
    f"python2 {args.GEMME_path}/gemme.py {MSA_upper_file} -r input -f {MSA_upper_file} "
    f"-m {mutant_temp_file} -N {args.nseqs}"
    )
    print(command)
    proc_obj = subprocess.run(command, shell=True, executable="/bin/bash", env=env, cwd=full_temp_folder, check=True)
    # parse output files 
    # find file with suffix _evolCombi.txt"java -Xmx9126m -cp $JET_PATH:$JET_PATH/jet/extLibs/vecmath.jar jet.JET -c default.conf
